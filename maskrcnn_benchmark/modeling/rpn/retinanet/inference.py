...
class RetinaNetPostProcessor(RPNPostProcessor):
    ...
    def forward_for_single_feature_map(
        ...
        per_candidate_inds, per_anchors in zip(
            ...
            if self.BBAM:
                proposals_return = per_anchors.bbox[per_box_loc, :].view(-1, 4)
                box_regression_return = per_box_regression[per_box_loc, :].view(-1, 4)
                class_logits_return = per_box_cls
                after_regression_proposal_return = detections
        ...
        if self.BBAM:
            return results, proposals_return, box_regression_return, class_logits_return, after_regression_proposal_return, per_class
        else:
            return results
...
def make_retinanet_postprocessor(config, rpn_box_coder, is_train, BBAM=False):
